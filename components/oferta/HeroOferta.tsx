    ...
    <>
      <div className="w-full overflow-hidden bg-gradient-to-r from-blue-600 to-cyan-500 py-2.5">
        <div className="flex animate-ticker whitespace-nowrap">
          {[...ticker, ...ticker].map((item, i) => (
            <span key={i} className="mx-10 text-xs font-semibold uppercase tracking-widest text-white">
              {item}
            </span>
          ))}
        </div>
      ...
      <section className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-6 py-24 text-center">
        ...
        <div className="relative z-10 mx-auto max-w-4xl">
          ...
          >
            <a href="#planos" className="rounded-xl bg-blue-600 px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-blue-600/40 transition hover:-translate-y-0.5 hover:bg-blue-500">
              Quero meu site agora →
            </a>
            <a href="#beneficios" className="rounded-xl border border-white/10 px-8 py-3.5 text-sm font-semibold text-white/80 transition hover:border-white/25 hover:text-white">
              Ver benefícios
            </a>
          </motion.div>
          ...
          >
            {stats.map((s) => (
              <div key={s.lbl} className="text-center">
                ...
                <div className="mt-1 text-xs font-semibold uppercase tracking-widest text-white/40">{s.lbl}</div>
              </div>
            ))}
          </motion.div>
        </div>
