...
const INPUT_CLASS =
  'w-full bg-[#0D0D0D] border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-gray-700 focus:outline-none focus:border-cyan-500/40 transition-all';
...
function TarefaModal({
  isOpen, onClose, onSuccess, tarefa,
}: {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (t: Tarefa) => void;
  tarefa?: Tarefa;
}) {
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({
    title: '', type: '', channel: '', due_at: '', done: false,
  });
  ...
  useEffect(() => {
    if (tarefa) {
      setForm({
        title: tarefa.title ?? '',
        type: tarefa.type ?? '',
        channel: tarefa.channel ?? '',
        due_at: tarefa.due_at ? tarefa.due_at.slice(0, 16) : '',
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#0C0C0C] border border-white/10 rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/5">
          <h2 className="font-outfit text-base font-black text-white">
        ...
        <div className="p-6 space-y-4">
          ...
          <label className="flex items-center gap-3 cursor-pointer group">
            <div
              ...
              className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${form.done ? 'bg-emerald-500/20 border-emerald-500/40' : 'bg-white/5 border-white/10'}`}
            >
        ...
        <div className="flex gap-3 px-6 pb-6">
          <button onClick={onClose} disabled={saving} className="flex-1 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-400 text-[11px] font-black uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all disabled:opacity-60">
            Cancelar
          </button>
          <button onClick={handleSubmit} disabled={saving || !form.title.trim()} className="flex-1 relative group disabled:opacity-60">
            ...
            <div className="relative flex items-center justify-center bg-white text-black py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all">
              {saving ? 'Salvando...' : tarefa ? 'Salvar' : 'Cadastrar'}
            </div>
          </button>
        </div>
      </div>
...
export default function TarefasPage() {
  ...
  const toggleDone = async (tarefa: Tarefa) => {
    ...
    const { data, error } = await supabase
      .from('tarefas').update({ done: !tarefa.done }).eq('id', tarefa.id).select().single();
  ...
  return (
    <div className="p-8 min-h-screen">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            {loading ? 'Carregando...' : `${filtered.length}${hasFilters ? ` de ${tarefas.length}` : ''} tarefa${tarefas.length !== 1 ? 's' : ''}`}
          </p>
        </div>
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        {hasFilters && (
          <button onClick={() => { setFilterType(''); setFilterDone(''); }} className="text-[11px] text-gray-600 hover:text-gray-400 transition-colors font-black uppercase tracking-widest">
            Limpar filtros
          </button>
        )}
      </div>
                  ...
                  <tr key={i} className="border-b border-white/5">
                    {Array.from({ length: 7 }).map((_, j) => (
                      <td key={j} className="px-5 py-4">
                        <div className="h-3 bg-white/5 rounded-lg animate-pulse" style={{ width: `${50 + (j * 17 + i * 11) % 40}%` }} />
                      </td>
                    ))}
                  </tr>
                ))
                  ...
                  <td colSpan={7} className="py-20">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-xs text-gray-700 mt-1">
                        {hasFilters ? 'Tente ajustar os filtros' : 'Adicione a primeira tarefa pelo botão acima'}
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                filtered.map(tarefa => (
                  <tr key={tarefa.id} className={`border-b border-white/5 last:border-0 transition-colors hover:bg-white/[0.025] ${tarefa.done ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-5 py-4">
                      <span className={`text-sm font-bold ${tarefa.done ? 'line-through text-gray-600' : 'text-white'}`}>
                        {tarefa.title || '—'}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      {tarefa.type ? <TypeBadge value={tarefa.type} /> : <span className="text-sm text-gray-700">—</span>}
                    </td>
                    ...
                    <td className="px-5 py-4 text-xs text-gray-600 whitespace-nowrap">
                      {tarefa.due_at ? formatDate(tarefa.due_at) : '—'}
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => setEditTarefa(tarefa)} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-cyan-500/10 border border-white/10 hover:border-cyan-500/20 flex items-center justify-center text-gray-600 hover:text-cyan-400 transition-all">
                          <Pencil className="w-3 h-3" />
                        </button>
                        <button onClick={() => setDeleteTarget(tarefa)} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-red-500/10 border border-white/10 hover:border-red-500/20 flex items-center justify-center text-gray-600 hover:text-red-400 transition-all">
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-[#0C0C0C] border border-white/10 rounded-2xl p-6 shadow-2xl">
            ...
            <p className="text-sm text-gray-400 mb-6">
              Tem certeza que deseja excluir <span className="font-bold text-white">{deleteTarget.title}</span>?
            </p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteTarget(null)} disabled={deleting} className="flex-1 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-400 text-[11px] font-black uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all disabled:opacity-60">
                Cancelar
              </button>
              <button onClick={handleDelete} disabled={deleting} className="flex-1 px-4 py-2.5 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 text-[11px] font-black uppercase tracking-widest transition-all disabled:opacity-60">
