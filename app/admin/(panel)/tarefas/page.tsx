...
const typeConfig: Record<string, { label: string; style: string }> = {
  ...
  outros:   { label: 'Outros',   style: 'bg-gray-500/15 text-secondary border-gray-500/20' },
};
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-surface border border-border rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-border">
          <h2 className="font-outfit text-base font-black text-primary">{tarefa ? 'Editar Tarefa' : 'Nova Tarefa'}</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-[9px] font-black uppercase tracking-widest text-secondary mb-1.5 block">Tipo</label>
              <select className={INPUT_CLASS} value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} style={{ backgroundColor: 'var(--color-inset)' }}>
                <option value="">Selecione</option>
                {Object.entries(typeConfig).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[9px] font-black uppercase tracking-widest text-secondary mb-1.5 block">Canal</label>
              <input className={INPUT_CLASS} placeholder="WhatsApp, Email..." value={form.channel} onChange={e => setForm(f => ({ ...f, channel: e.target.value }))} />
            </div>
          </div>
          <div>
            <label className="text-[9px] font-black uppercase tracking-widest text-secondary mb-1.5 block">Prazo</label>
          ...
          <label className="flex items-center gap-3 cursor-pointer group">
            <div onClick={() => setForm(f => ({ ...f, done: !f.done }))} className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${form.done ? 'bg-emerald-500/20 border-emerald-500/40' : 'bg-white/5 border-border'}`}>
              {form.done && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />}
            </div>
            <span className="text-sm text-secondary group-hover:text-secondary transition-colors">Marcar como concluída</span>
          </label>
        </div>
  ...
  return (
    <div className="p-8 min-h-screen">
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="font-outfit text-3xl font-black tracking-tight text-primary">Tarefas</h1>
          <p className="text-secondary text-sm mt-1">{loading ? 'Carregando...' : `${filtered.length}${hasFilters ? ` de ${tarefas.length}` : ''} tarefa${tarefas.length !== 1 ? 's' : ''}`}</p>
        </div>
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <select value={filterType} onChange={e => setFilterType(e.target.value)} className={SELECT_CLASS} style={{ backgroundColor: 'var(--color-inset)' }}>
          <option value="">Todos os tipos</option>
          {Object.entries(typeConfig).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
        </select>
        <select value={filterDone} onChange={e => setFilterDone(e.target.value)} className={SELECT_CLASS} style={{ backgroundColor: 'var(--color-inset)' }}>
          <option value="">Todas as tarefas</option>
          <option value="pendente">Pendentes</option>
          <option value="concluida">Concluídas</option>
        </select>
        {hasFilters && <button onClick={() => { setFilterType(''); setFilterDone(''); }} className="text-[11px] text-secondary hover:text-secondary transition-colors font-black uppercase tracking-widest">Limpar filtros</button>}
      </div>
              ...
              {loading ? (
                ...
                <tr><td colSpan={7} className="py-20">
                  <div className="flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm font-bold text-secondary">Nenhuma tarefa encontrada</p>
                    <p className="text-xs text-muted mt-1">{hasFilters ? 'Tente ajustar os filtros' : 'Adicione a primeira tarefa pelo botão acima'}</p>
                  </div>
                </td></tr>
              ) : (
                filtered.map(tarefa => (
                  <tr key={tarefa.id} className={`border-b border-border last:border-0 transition-colors hover:bg-white/[0.025] ${tarefa.done ? 'opacity-50' : ''}`}>
                    <td className="px-5 py-4 w-10">
                      <button onClick={() => toggleDone(tarefa)} className="text-secondary hover:text-emerald-400 transition-colors">
                        {tarefa.done ? <CheckCircle2 className="w-4 h-4 text-emerald-400" /> : <Circle className="w-4 h-4" />}
                      </button>
                    </td>
                    <td className="px-5 py-4"><span className={`text-sm font-bold ${tarefa.done ? 'line-through text-secondary' : 'text-primary'}`}>{tarefa.title || '—'}</span></td>
                    <td className="px-5 py-4">{tarefa.type ? <TypeBadge value={tarefa.type} /> : <span className="text-sm text-muted">—</span>}</td>
                    <td className="px-5 py-4 text-sm text-secondary">{tarefa.channel || '—'}</td>
                    <td className="px-5 py-4 text-xs text-secondary whitespace-nowrap">{tarefa.due_at ? formatDate(tarefa.due_at) : '—'}</td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => setEditTarefa(tarefa)} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 border border-border hover:border-border-strong flex items-center justify-center text-secondary hover:text-primary transition-all"><Pencil className="w-3 h-3" /></button>
                        <button onClick={() => setDeleteTarget(tarefa)} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-red-500/10 border border-border hover:border-red-500/20 flex items-center justify-center text-secondary hover:text-red-400 transition-all"><Trash2 className="w-3 h-3" /></button>
                      </div>
                    </td>
                  </tr>
                ))
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-surface border border-border rounded-2xl p-6 shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-outfit text-base font-black text-primary">Excluir Tarefa</h3>
